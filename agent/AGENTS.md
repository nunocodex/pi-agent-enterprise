...
## 10. Ephemeral Artifact Management

This section defines the formal policy for all temporary, session‑specific, and cached files generated by the agentic workflow.

### 10.1 Directory Structure & Hierarchy
The ephemeral workspace is organized as follows:

    .pi/
    ├── tmp/                     # Ephemeral (volatile) storage
    │   ├── current_session      # Single file storing the active SESSION_ID
    │   ├── cache/               # Shared cache across sessions (TTL: 1 hour)
    │   │   └── explore_{hash}.json
    │   └── {SESSION_ID}/        # Session‑isolated directory
    │       ├── lock/            # Execution locks (TTL: 30 minutes)
    │       │   └── execute.lock
    │       ├── artifacts/       # Build artifacts, compiled outputs, test cache
    │       ├── plan_draft.md    # Draft plan awaiting confirmation
    │       ├── execution.log    # Execution logs with timestamps
    │       ├── review_report.json # Structured review output
    │       └── coverage/        # Test coverage reports
    └── state/                   # Persistent storage (version‑controlled)
        ├── PLAN.md              # Source of truth for the project plan
        ├── MEMORY.md            # Persistent agent memory
        └── AGENTS.md            # This file

### 10.2 Session Lifecycle
1. **Creation**: `/init` generates a unique `SESSION_ID` (timestamp + UUID) and writes it to `.pi/tmp/current_session`.
2. **Usage**: All subsequent commands read `SESSION_ID` from `.pi/tmp/current_session` to locate the session‑isolated workspace.
3. **Termination**: `/abort` removes `.pi/tmp/{SESSION_ID}/`, deletes `.pi/tmp/current_session`, and cleans stale caches.
4. **Idempotency**: If `SESSION_ID` is missing, commands treat it as "no active session" and halt gracefully.

### 10.3 TTL (Time‑To‑Live) Policies
| Artifact Type | Location | TTL | Cleanup Trigger |
|---------------|----------|-----|-----------------|
| Exploration cache | `.pi/tmp/cache/` | 1 hour | `/explore` checks age; `/abort` removes >24h |
| Build artifacts | `.pi/tmp/{SESSION_ID}/artifacts/` | Session lifetime | `/abort` removes entire session |
| Execution locks | `.pi/tmp/{SESSION_ID}/lock/` | 30 minutes | `/init` removes stale locks; `/execute` checks |
| Session logs & drafts | `.pi/tmp/{SESSION_ID}/` | Session lifetime | `/abort` removes entire session |
| Coverage reports | `.pi/tmp/{SESSION_ID}/coverage/` | Session lifetime | `/abort` removes entire session |

### 10.4 Security & Access Control
- **Permissions**: All ephemeral directories must have `700` (or `600`) permissions to prevent other system users from accessing sensitive data.
- **Data Isolation**: Session directories are isolated by UUID; no cross‑session data leakage.
- **Sensitive Data**: Logs, artifacts, and reports must **never** contain PII, tokens, credentials, or database credentials. Use structured logging with redaction where necessary.

### 10.5 Atomic Operations
- **File Writes**: All writes to temporary files must be atomic:
  1. Write to a temporary file (e.g., `plan_draft.md.tmp`).
  2. Validate the content (e.g., markdown syntax).
  3. Rename (`mv`) the temporary file to the final name.
- This prevents partial reads or corrupted files.

### 10.6 Automated Cleanup
- `/abort` explicitly removes the current session directory.
- `/init` checks for stale locks (`*.lock` older than 30 minutes) and removes them.
- A periodic (daily) cron job can be configured to remove `*.tmp` directories older than 24 hours using `find .pi/tmp -type d -mtime +1 -exec rm -rf {} \;`.

### 10.7 Integration with Commands
| Command | Ephemeral Artifacts Used | Behavior |
|---------|--------------------------|----------|
| `/init` | Creates session, validates environment, cleans stale locks | Writes `SESSION_ID` to `.pi/tmp/current_session` |
| `/explore` | Reads/writes `cache/explore_{hash}.json` | Uses cache if valid; otherwise performs fresh scan |
| `/plan` | Writes `plan_draft.md` in session | Moves to `.pi/state/PLAN.md` upon confirmation |
| `/execute` | Uses `lock/execute.lock`, `execution.log`, `artifacts/` | Acquires lock; releases after DoD |
| `/review` | Writes `review_report.json` in session | Produces structured JSON for diff tracking |
| `/test` | Writes `coverage/` and `test_summary.json` in session | Stores coverage data for analysis |
| `/abort` | Removes entire `SESSION_ID` directory and `current_session` | Purges all ephemeral data |
| `/commit` | Reads `SESSION_ID` for traceability | Does not create/modify ephemeral files |
